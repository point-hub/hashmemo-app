...
import { archiveDocumentApi } from '@/composables/api/documents/archive.api';
...
const onArchive = async (reason: string) => {
  ...
  try {
    await archiveDocumentApi(_id.value as string, {
      update_reason: reason,
    });
    toast(`Archive Document "${label.value}" success`, { color: 'success' });
    emit('archived');
  } catch (error) {
