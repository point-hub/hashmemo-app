...
const props = withDefaults(
  ...
  { currentUser: null, signaturesJson: '' },
);
...
const onSign = (sig: ISignature) => {
  if (props.currentUser?.user_id !== sig.user_id) {
    return;
  }
  sig.signed = true;
  updateSignaturesJson();
  emit('signature:signed', sig);
};
    ...
    >
      ...
      >
        <button
          ...
          v-if="!sig.signed"
          @click.stop="onSign(sig)"
        >
          <div class="flex flex-col w-28 h-18 items-center justify-between">
            <div class="text-sm font-bold">
              SIGN
            </div>
            <div v-if="currentUser?.user_id === sig.user_id" class="text-xs text-blue-600 bg-blue-300 px-3 py-0.5">click</div>
