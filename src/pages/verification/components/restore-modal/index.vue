...
import { restoreDocumentApi } from '@/composables/api/documents/restore.api';
...
const onRestore = async (reason: string) => {
  ...
  try {
    await restoreDocumentApi(_id.value as string, {
      update_reason: reason,
    });
    toast(`Restore Document "${label.value}" success`, { color: 'success' });
    emit('restored');
  } catch (error) {
