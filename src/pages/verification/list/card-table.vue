 ...
 * - documents data retrieved from API
 * - loading state
 * - control flags to prevent unnecessary watcher triggers
 */
const documents = ref<IDocumentData[]>();
...
const onPageUpdate = async () => {
  if (!isInitialSetup.value) {
    await getDocuments(pagination.page);
 ...
 * @param document - The data row to delete
 * @param index - Index of the row for UI references
 */
const onDeleteModal = (document: IDocumentData, index: number) => {
  ...
  <base-card title="Documents">
    <div class="flex flex-col lg:flex-row gap-2 items-center justify-between mb-8">
      ...
      <div class="flex gap-1">
        <router-link to="/master/documents/create">
    ...
    <div class="flex flex-col gap-4">
      <base-table>
        ...
        <tbody>
          ...
          <!-- Show no data found message if no documents and query params exist -->
          <tr v-if="!isLoading && documents?.length === 0 && route.query">
            <td :colspan="countVisibleColumns + 1">
          ...
          <!-- Render rows of document data when available -->
          <template v-if="!isLoading && documents && documents.length > 0">
            <tr v-for="(document, index) in documents" :key="index">
                  ...
                  <template #content>
                    <base-card class="p-0! gap-0! -mt-2" shadow>
                      <div class="flex flex-col">
                        <router-link :to="`/master/documents/${document._id}`">
                        ...
                        <router-link :to="`/master/documents/${document._id}/edit`">
                          <base-button variant="text" color="info" class="w-full py-1! px-3! m-0! flex gap-2! items-center justify-start text-left!">
                        ...
                        <base-divider orientation="vertical" class="my-0!" />
                        <base-button @click="onDeleteModal(document, index)" variant="text" color="danger" class="w-full py-1! px-3! m-0! flex gap-2! items-center justify-start text-left!">
              ...
              <!-- Document fields rendered conditionally based on column visibility -->
              <td v-if="columns['code']?.isVisible">
                <router-link :to="`/master/documents/${document._id}`" class="text-blue">{{ document.code }}</router-link>
              </td>
              <td v-if="columns['name']?.isVisible">
                <router-link :to="`/master/documents/${document._id}`" class="text-blue">{{ document.name }}</router-link>
              </td>
              <td v-if="columns['composite_unique_1']?.isVisible">{{ document.composite_unique_1 }}</td>
              <td v-if="columns['composite_unique_2']?.isVisible">{{ document.composite_unique_2 }}</td>
              <td v-if="columns['age']?.isVisible">{{ document.age }}</td>
              <td v-if="columns['gender']?.isVisible">{{ document.gender }}</td>
              <td v-if="columns['optional_unique']?.isVisible">{{ document.optional_unique }}</td>
              <td v-if="columns['notes']?.isVisible">{{ document.notes }}</td>
              <td v-if="columns['is_archived']?.isVisible">
                <base-badge v-if="document.is_archived" variant="filled" color="danger" class="font-bold">
