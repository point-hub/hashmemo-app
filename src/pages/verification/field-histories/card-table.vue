      ...
      <base-table>
        ...
        <tbody>
          ...
          <template v-if="!isLoading && auditLogs && auditLogs.length > 0">
            <tr v-for="(document, index) in auditLogs" :key="index">
              <!-- Fields rendered conditionally based on column visibility -->
              <td v-if="columns['created_at']?.isVisible" class="w-40">{{ formatDate(document.created_at) }}</td>
              <td v-if="columns['operation_id']?.isVisible">
                <router-link :to="`/administrator/audit-logs/${document.operation_id}`" class="text-blue">{{ document.operation_id?.substring(0, 13) }}...</router-link>
              </td>
              <td v-if="columns['actor']?.isVisible">
                <router-link :to="`/master/users/${document.actor_id}`" class="text-blue">{{ document.actor_name }}</router-link>
              </td>
              <td v-if="columns['before']?.isVisible">{{ document.before }}</td>
              <td v-if="columns['after']?.isVisible">{{ document.after }}</td>
            </tr>
          </template>
        </tbody>
      </base-table>
